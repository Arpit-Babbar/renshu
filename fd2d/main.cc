...
private:
    ...
    double exponential_function(double grid_point);
    ...
    for (unsigned int i = 0; i < n_points; i++)
      for (unsigned int j = 0; j < n_points; j++)
      {
          ...
          case 2:
              ...
              break;
          case 3:
              solution(i,j) = exponential_function(x)*exponential_function(y);
              break;
          default:
    ...
    for (unsigned int i = 0; i < n_points; i++)
      for (unsigned int j = 0; j < n_points; j++)
      {
          ...
          case 2:
              ...
              break;
          case 3:
              solution_exact(i,j) = exponential_function(x-coefficient_x*t)
                                    * exponential_function(y-coefficient_y*t);
              break;
          default:
              cout << "You entered the wrong initial_data_indicator ";
              assert(false);
                        ...
                        unsigned int max_refinements)
{
    ...
    if (snapshot_vector[max_refinements-1]-(-1.0) < 1e-12)
    {
    ...
    cout << "The L_infty snapshot error is " <<snapshot_vector[max_refinements-1]<<endl;
    cout << endl;         
}
...
double Linear_Convection_2d::exponential_function(double grid_point)
{
  grid_point = interval_part(grid_point);
